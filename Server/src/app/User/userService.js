...
exports.createSocialUser = async function (email,nickname) {
    try {

        const connection = await pool.getConnection(async (conn) => conn);
        const userIdResult = await userDao.insertSocialUser(connection, email, nickname);
        connection.release();
...
exports.postSocialSignIn = async function (email) {
    try {
        ...
        const connection = await pool.getConnection(async (conn) => conn);
        const tokenResult = await userDao.insertToken(connection, userInfoRows[0].userId, token);
        connection.release();

        return response(baseResponse.USER_SIGNIN_SUCCESS, {'userId': userInfoRows[0].userId, 'jwt': token});
